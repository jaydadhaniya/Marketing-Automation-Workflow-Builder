    ...
    <a-table :columns="store.columns" :data-source="workflows" :showSorterTooltip="false">
      <template #headerCell="{ column }">
        <template v-if="column.key === 'sharing'">
          <a-tooltip>
            <template #title>
              <div>Private: Only visible to you.</div>
              <div>Shared: Visible to everyone.</div>
            </template>
            Sharing
      ...
      <template #bodyCell="{ column, record: workflow }">
        ...
        <template v-else-if="column.key === 'sharing'">
          <a-tag
            :color="workflow.sharing === 'private' ? 'red' : 'green'"
        ...
        <template v-else-if="column.key === 'workflowType'">
          ...
          <a-tag v-else color="purple" style="font-size: 14px" class="p-1"> Single Trigger </a-tag>
        </template>
